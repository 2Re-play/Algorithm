    ...
    private static void solution(int n, int[][] delivery) {
        ...
        for(int i=0; i<delivery.length; i++) {
            ...
            if(delivery[i][2] == 0) { // 미배송
                ...
                } else if (noRemindProduct.contains(delivery[i][0]) && !remindProduct.contains(delivery[i][1]) && !noRemindProduct.contains(delivery[i][1])) {
                    sb.append("?");
                } else if (noRemindProduct.contains(delivery[i][1]) && !remindProduct.contains(delivery[i][0]) && !noRemindProduct.contains(delivery[i][0])) {
                    sb.append("?");
                }
            }
